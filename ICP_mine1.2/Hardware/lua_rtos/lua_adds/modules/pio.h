...
#define DECLARE_is_gpio_pin(pin)   ( ((pin>=2)&&(pin<=10)) || \
													   ((pin>=PIO_A0)&&(pin<=PIO_LED1)) )
#define DECLARE_is_analog_pin(pin) ( ((pin>=PIO_A0)&&(pin<=PIO_A5)) )
...
enum {GPIO_INPUT,GPIO_OUTPUT_PP,GPIO_ANALOG,GPIO_OUTPUT_OD};
//enum {GPIO_PULLNO,GPIO_PULLUP,GPIO_PULLDOWN};
#define DECLARE_is_gpio_mode(mode) ( ((mode)>=GPIO_INPUT)&&((mode)<=GPIO_OUTPUT_OD) )
#define DECLARE_is_gpio_pull(pull) ( ((pull)>=GPIO_NOPULL)&&((pull)<=GPIO_PULLDOWN) )
 ...
 */
 ...
 #define DECLARE_GPIO_INIT(name,mode,updown,speed) \
	GPIO_InitTypeDef name ={ \
		  .Pin=0, \
			.Mode=GPIO_MODE_##mode, \
			.Pull=GPIO_##updown,\
			.Speed=GPIO_SPEED_FREQ_##speed,\
	};
